...
function custom_every_minute_schedule( $schedules ) {
    ...
    $schedules['everyday'] = array(
        'interval' => 86400,
        'display'  => __( 'Custom Every Day', 'creatotech-dynamic-news' ),
    );
    return $schedules;
}
...
if ( ! wp_next_scheduled( 'custom_every_minute_event' ) ) {
    wp_schedule_event( time(), 'everyday', 'custom_every_minute_event' );
}
...
function custom_every_minute_cronjob() {
   ...
   //require plugin_dir_path( __FILE__ ) . 'admin/partials/cron-creatotech-dynamic-news-admin-display.php';
